...
export class PromotionModalComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        private _matDialogRef: MatDialogRef<PromotionModalComponent>,
    ...
    async uploadImageAndPatchForm(): Promise<void> {
        if (this.selectedImage) {
            try {
                ...
                const imageUrl = await this._portalService.uploadFileWithoutId(this.selectedImage, 'promotions');
    ...
    async save(): Promise<void> {
        ...
        if (this.form.valid) {
            ...
            try {
                if (this.single === 'add') {
                    const res = await this._portalService.createPromotion(formData);
                    console.log('Promotion created successfully with ID:', res.id);
                    this._matDialogRef.close({ ...res, id: res.id });
                } else {
                    const res = await this._portalService.updatePromotion(this.single.id, formData);
                    console.log('Promotion updated successfully!');
                    this._matDialogRef.close(res);
                }
            } catch (error) {
                console.error('Error saving promotion:', error);
                this.error = 'Failed to save promotion. Please try again.';
            } finally {
                this.loading = false;
                this.cdr.detectChanges();
            }
